	...
	return {
		...
		"move_head" : function() {
			...
			if (next_field === grower_food) {
				...
				$("#snake_game_field > div.bonus_food").remove();
		...
		*/
		...
		"generate_random_field_element" : function() {
			...
			var index = left + "_" + top;
			if (obstacles[index] || bonus_food == index || grower_food == index) {return this.generate_random_field_element();}
			return [left, top];
		},
		...
		"add_new_food" : function(option) {
			...
			if (option === 1) {
				...
				$(new_food).addClass("grower_food");
			}
			else if (option === 2) {
				bonus_food = coordinates[0] + "_" + coordinates[1];
				$(new_food).addClass("bonus_food");
			}			
		...
		"draw_X" : function(start_x, start_y, thickness) {
			...
			if (field_width > field_height) {
				...
				for (var y = 0; y < h_limbs; y++) {
				 ...
				 // deficit of x over y in terms of normal build
				//console.log("Deficit: " + deficit);
				var a111_was_one = 0;
				var a111 = Math.floor(h_limbs / deficit);
				
				if (a111 === 1) {a111_was_one = 1; a111++;}
				// compute xtra x
				var x_extra_counter = 0;		
				
				
				var divisor = Math.floor(h_limbs / deficit);
				
				/*var compute_xtra_x = function(deficit, sign) {
					if (typeof(sign) === "undefined") {sign = "+";}
					if (divisor === 1) {
						for (var y = 0; y < h_limbs; y++) {
							xtra_x[y] = 1;
						}
						compute_xtra_x(h_limbs - deficit, "-");
						return;
					}
					
					var counter = 0;
					for (var y = 0; y < half_length; y++) {
						if (counter < deficit && (y+1) % divisor === 0) {
							if (sign === "+") {xtra_x[y] = 1;}
				...
				//if (half_length < deficit) {
				/*if (2===3) {
					for (var y = 0; y < half_length; y++) {
						var sym_y = h_limbs - (y + 1);
						xtra_x[y] = false;
						xtra_x[sym_y] = false;
						if (x_extra_counter <= deficit) {
							xtra_x[y] = true;
							x_extra_counter++;
						}
						if (x_extra_counter <= deficit) {
					...
					for (var y = 0; y < half_length; y++) {
					var sym_y = h_limbs - (y + 1);
					if ((y+1) % a111 === 0 && x_extra_counter <= deficit) {
						xtra_x[y] = true;
						x_extra_counter++;
						if (x_extra_counter <= deficit) {
							xtra_x[sym_y] = true;
					...
					if (a111_was_one || deficit % 2 !== 0) {xtra_x[0] = true;}
					//if (deficit % 2 !== 0) {xtra_x[0] = true;}
				}	*/			
